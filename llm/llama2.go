...
func NewBuiltinModel() *Model {
	m, err := NewModel(bytes.NewReader(checkpoint), bytes.NewReader(tokenizer))
	if err != nil {
		panic(err)
	}
...
func NewModel(checkpoint, tokfile io.Reader) (*Model, error) {
	var config Config
	...
	checkpointInitWeights(&weights, &config, checkpoint)
...
func LoadModel(checkpoint, tokenizer string) (*Model, error) {
...
func (m *Model) Inference(prompt string, temperature float32, seed int64, steps int32) (string, error) {
	...
	*/
	...
	if steps <= 0 || steps > m.config.SeqLen {
		steps = m.config.SeqLen
	}
	...
	for pos < steps {
		...
		transformer(token, pos, &m.config, &state, &m.weights)
		...
		if pos < int32(len(promptTokens)) {
			...
			if temperature == 0.0 {
				...
				for q := int32(0); q < m.config.VocabSize; q++ {
